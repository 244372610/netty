...
public class DefaultDnsRecordEncoderTest {
    ...
    public void testIpv4() throws Exception {
        testIp(InetAddress.getByName("1.2.3.4"));
    }

    @Test
    public void testIpv6() throws Exception {
        testIp(InetAddress.getByName("::0"));
    }

    private static void testIp(InetAddress address) throws Exception {
        int addressBits = address.getAddress().length * Byte.SIZE;
        for (int i = 1; i <= addressBits; i += 1) {
            testIp(address, i);
        }
    }
    ...
    private static void testIp(InetAddress address, int prefix) throws Exception {
        ...
        int prefixRounded = (int) (Math.rint((double) prefix / Byte.SIZE) * Byte.SIZE) / Byte.SIZE;
        ByteBuf addressPart = Unpooled.wrappedBuffer(address.getAddress(), 0, Math.max(1, prefixRounded));

        int leftOverBits = prefix % Byte.SIZE;
        if (leftOverBits > 0) {
            // Pad the leftover of the last byte with zeros.
            int idx = addressPart.writerIndex() - 1;
            byte lastByte = addressPart.getByte(idx);
            addressPart.setByte(idx, DefaultDnsRecordEncoder.padWithZeros(lastByte, leftOverBits));
        }
        ...
        try {
            ...
            assertEquals(addressPart.readByte(), out.readByte());
